...
def select_video_to_download(vids_to_dl, url_storage):
    while True:
...
def download_videos(vids_to_dl, url_storage, datestr, by_url):
    # used_videos = set()
    path = "videos/raw/" + datestr
    dl_ct = NUM_OF_VIDEOS
    # f = open(filename, 'a')
    while dl_ct > 0:
        vid = select_video_to_download(vids_to_dl, url_storage)
        title = fix_title(by_url[vid], ILLEGAL_CHARACTERS) + '.mp4'
        try:
            download_video(vid, path, title)
            dl_ct -= 1
            # used_videos.add(vid)
